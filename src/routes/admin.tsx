...
function Login({ onSuccess }: { onSuccess: () => void }) {
  ...
  return (
    <div className="admin-wrap" style={{ display: "grid", placeItems: "center", padding: 20 }}>
      <style>{GLOBAL_CSS + css}</style>
      <div className="panel" style={{ width: "100%", maxWidth: 420, padding: 28 }}>
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 15 }}>
          <LGLogo size={70} showText={false} />
        </div>
        <div style={{ textAlign: "center", marginBottom: 22 }}>
          <div className="admin-title">Admin Portal</div>
          <div className="admin-sub">Learner's Guide administration</div>
        </div>
...
function Dashboard({ user, onLogout }: { user: User; onLogout: () => void }) {
  ...
  const load = async () => {
    ...
    const first = s.error || t.error || a.error;
  ...
  const counts = useMemo(
    ...
    [students, teachers, ann],
  );

  const saveStudent = async () => {
    if (!studentForm.name || !studentForm.sid || !studentForm.cls || !studentForm.sec)
      return setError("Student name, SID, class and section are required.");
    const { error: e } = await supabase
      .from("students")
      .insert({ id: `s${Date.now()}`, ...studentForm, status: "active" });
    if (e) setError(e.message);
    else {
      setNotice("Student profile created.");
      setShowStudent(false);
      setStudentForm({ name: "", sid: "", cls: "", sec: "", parentname: "", parentphone: "" });
      load();
    }
  };

  const saveTeacher = async () => {
    if (!teacherForm.name || !teacherForm.tid || !teacherForm.phone)
      return setError("Teacher name, teacher ID and phone are required.");
    const { error: e } = await supabase
      .from("teachers")
      .insert({ id: `t${Date.now()}`, ...teacherForm, classes: [], status: "active" });
    if (e) setError(e.message);
    else {
      setNotice("Teacher profile created.");
      setShowTeacher(false);
      setTeacherForm({ name: "", tid: "", subject: "", phone: "" });
      load();
    }
  };

  const saveAnnouncement = async () => {
    if (!announcementForm.title) return setError("Announcement title is required.");
    ...
    if (e) setError(e.message);
      ...
      setAnnouncementForm({ title: "", desc: "", target: "all" });
      load();
    }
  };
      ...
      <div className="admin-shell">
        <aside className="admin-side">
          <div className="admin-brand">
            <LGLogo size={42} showText={false} light />
            <b>
              Learner's Guide
              <br />
              <span style={{ opacity: 0.5, fontSize: 10 }}>ADMIN CONSOLE</span>
            </b>
          </div>
        ...
        <main className="admin-main">
          <div className="admin-top">
            <div>
              <div className="admin-title">{nav.find((n) => n[0] === tab)?.[1]}</div>
              <div className="admin-sub">Signed in as administrator · {user.name}</div>
            </div>
            ...
            <>
              ...
              <div className="panel">
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 9 }}>
                  <button className="btn btn-primary" onClick={() => setShowStudent(true)}>
                    + Add Student
                  </button>
                  <button className="btn btn-primary" onClick={() => setShowTeacher(true)}>
                    + Add Teacher
                  </button>
              ...
              <div className="panel">
                ...
                <StudentTable rows={students.slice(0, 8)} onDelete={removeStudent} />
              </div>
            </>
          )}
          {tab === "students" && (
            <div className="panel">
              ...
              <StudentTable rows={students} onDelete={removeStudent} />
            </div>
          )}
          {tab === "teachers" && (
            <div className="panel">
              ...
              <TeacherTable rows={teachers} onDelete={removeTeacher} />
            </div>
          )}
          {tab === "announcements" && (
            <div className="panel">
              ...
              <AnnouncementTable rows={ann} />
            </div>
          )}
          {showStudent && (
            <Modal title="Add Student" onClose={() => setShowStudent(false)}>
              <div className="form-grid">
                {Object.entries(studentForm).map(([k, v]) => (
                  <div className="field" key={k}>
                    ...
                    <input
                      value={v}
                      onChange={(e) => setStudentForm({ ...studentForm, [k]: e.target.value })}
                    />
                  </div>
                ))}
              </div>
          ...
          {showTeacher && (
            <Modal title="Add Teacher" onClose={() => setShowTeacher(false)}>
              <div className="form-grid">
                {Object.entries(teacherForm).map(([k, v]) => (
                  <div className="field" key={k}>
                    <label>{k.toUpperCase()}</label>
                    <input
                      value={v}
                      onChange={(e) => setTeacherForm({ ...teacherForm, [k]: e.target.value })}
                    />
                  </div>
                ))}
              </div>
          ...
          {showAnnouncement && (
            <Modal title="Publish Announcement" onClose={() => setShowAnnouncement(false)}>
              <div className="form-grid">
                <div className="field span-2">
                  <label>TITLE</label>
                  <input
                ...
                <div className="field span-2">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setAnnouncementForm({ ...announcementForm, desc: e.target.value })
                    }
                  />
                </div>
                <div className="field">
        ...
        <tbody>
          {rows.map((s) => (
            <tr key={s.id}>
              ...
              <td>{s.parentname || "—"}</td>
              <td>
                <span className="pill">{s.status || "active"}</span>
              </td>
              <td>
        ...
        <tbody>
          {rows.map((t) => (
            <tr key={t.id}>
              ...
              <td>{t.phone}</td>
              <td>
                <span className="pill">{t.status || "active"}</span>
              </td>
              <td>
...
function AnnouncementTable({ rows }: { rows: Announcement[] }) {
  return (
    <div className="table-wrap">
      <table className="table">
        <thead>
        ...
        <tbody>
          {rows.map((a) => (
            <tr key={a.id}>
              ...
              <td style={{ whiteSpace: "normal", minWidth: 260 }}>{a.desc || "—"}</td>
              <td>
                <span className="pill">{a.target || "all"}</span>
              </td>
              <td>{a.date || "—"}</td>
            </tr>
          ))}
...
function Modal({
  ...
  return (
    ...
    >
      <div
        className="panel"
        style={{ width: "100%", maxWidth: 620, maxHeight: "90vh", overflow: "auto" }}
      >
        <div className="panel-head">
          <h2>{title}</h2>
