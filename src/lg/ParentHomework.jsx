...
async function getPdf(id,filename){
  ...
  const res=await fetch(`${SB_URL}/functions/v1/homework-file`,{method:"POST",headers:{Authorization:`Bearer ${session.access_token}`,apikey:SB_KEY,"Content-Type":"application/json"},body:JSON.stringify({id})});
  if(!res.ok){let message="Unable to download homework PDF.";try{const body=await res.json();if(body?.error)message=body.error}catch{}throw new Error(message)}
  const type=res.headers.get("content-type")||"";
  if(!type.toLowerCase().includes("pdf")&&!type.toLowerCase().includes("octet-stream"))throw new Error("The homework file service did not return a PDF.");
