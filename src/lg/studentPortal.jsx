...
async function queryStudentData(student, resource) {
  if (!student) return [];
  ...
  switch (resource) {
    ...
    case "homework": {
      ...
      const { data, error } = batch
        ? await query.eq("batch_id", batch)
        : await query.eq("cls", cls).eq("sec", sec);
      if (error) throw error;
      return data || [];
    }
    case "materials": {
      ...
      // Supabase RLS remains the authoritative access control boundary.
      // The client only keeps records matching the student's Standard when
      // legacy rows still expose class metadata, avoiding the old full-table
      // browser-side batch/class/section filtering.
      const standard = cls.toLowerCase();
      const folders = (foldersResult.data || []).filter((folder) => {
        const access = Array.isArray(folder.access_standards)
          ? folder.access_standards.map((value) => text(value).toLowerCase())
          : null;
        return !access || access.length === 0 || access.includes(standard);
      });
      const folderIds = new Set(folders.map((folder) => folder.id));
      const materials = (materialsResult.data || []).filter((material) => {
        if (material.folder_id) return folderIds.has(material.folder_id);
        const mc = text(material.cls);
        const ms = text(material.sec);
        return (!mc || mc === cls) && (!ms || ms === sec);
      });
    ...
    case "timetable": {
      try {
        const rows = await gdb("timetable");
        return (Array.isArray(rows) ? rows : []).filter((row) =>
          batch && row.batchId
            ? String(row.batchId) === batch
            : String(row.cls) === cls && String(row.sec) === sec,
        );
      } catch (error) {
        throw error;
      }
...
export function STMaterials({ student }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const d = await queryStudentData(student, "materials");
        if (live) {
