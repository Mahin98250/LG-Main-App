...
const ADMIN_CSS = `@import url('https://fonts.googleapis.com/css2?family=Poppins:wght@400;500;600;700;800;900&display=swap');*{box-sizing:border-box}body{font-family:'Poppins',sans-serif;background:${C.bg}}input,select,textarea{font-family:'Poppins',sans-serif}@keyframes pulse{0%,100%{transform:scale(1)}50%{transform:scale(1.05)}}.btn{transition:transform .18s cubic-bezier(.34,1.56,.64,1),box-shadow .18s ease,filter .15s ease;cursor:pointer}.btn:hover{transform:translateY(-2px) scale(1.02);box-shadow:0 8px 24px rgba(67,97,238,.18);filter:brightness(1.06)}.nav-item{transition:all .2s cubic-bezier(.34,1.56,.64,1);cursor:pointer;border-radius:12px}.nav-item:hover{background:rgba(255,255,255,.08)!important;transform:translateX(4px)}.nav-item.active{background:${C.accent}!important;box-shadow:0 6px 20px rgba(67,97,238,.35)}`;
...
function Sidebar({ activePage, onNavigate, onLogout }: Pick<Props, "activePage" | "onNavigate" | "onLogout">) {
  return <aside style={{width:230,minHeight:"100vh",background:C.sidebar,display:"flex",flexDirection:"column",padding:"0 12px 20px",flexShrink:0,position:"sticky",top:0}}>
    <div style={{padding:"20px 8px 16px",borderBottom:"1px solid rgba(255,255,255,.08)"}}><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}><div style={{width:40,height:40,borderRadius:12,background:"#fff",display:"flex",alignItems:"center",justifyContent:"center",padding:4}}><LGLogo size={32} showText={false}/></div><div><div style={{fontSize:16,fontWeight:900,color:"#fff"}}>Learner&apos;s</div><div style={{fontSize:16,fontWeight:900,color:C.gold,marginTop:-3}}>Guide</div></div></div><div style={{display:"inline-flex",alignItems:"center",gap:6,background:"rgba(239,68,68,.18)",borderRadius:20,padding:"3px 10px"}}><div style={{width:7,height:7,borderRadius:"50%",background:"#EF4444",animation:"pulse 1.5s infinite"}}/><span style={{fontSize:11,color:"#EF4444",fontWeight:700}}>ADMIN PANEL</span></div></div>
