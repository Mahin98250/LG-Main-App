...
export default function AdminRecordsPage({ kind }: { kind: Kind }) {
  ...
  const openNew = async () => {
    ...
    try {
      await ensureAdminSession();
      if (isStudent) setForm({ name: "", sid: await nextId("students"), tid: "", cls: "", sec: "", enroll: new Date().toISOString().slice(0, 10), status: "active", password: generateTemporaryPassword(), parentPassword: generateTemporaryPassword(), parentName: "", parentPhone: "", phone: "" });
      else setForm({ name: "", sid: "", tid: await nextId("teachers"), cls: "", sec: "", enroll: "", status: "active", password: generateTemporaryPassword(), parentPassword: "", parentName: "", parentPhone: "", phone: "" });
      setModal(true);
