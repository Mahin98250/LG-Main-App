...
// Supabase Auth requires a real password; weak 4-character defaults were causing
// account creation to fail before the teacher/student record could be linked.
...
// Common CBSE subjects for Classes 9-12, including the Commerce subjects used
// across Classes 11-12. Admin can assign multiple subjects to one teacher.
...
function MultiSelect({ label, value, onChange, options }: { label: string; value: string[]; onChange: (v: string[]) => void; options: string[] }) {
  return <div><span style={{ display: "block", fontSize: 12, fontWeight: 750, color: C.sub, marginBottom: 6 }}>{label}</span><div style={{ display: "flex", flexWrap: "wrap", gap: 7, padding: 10, border: `1.5px solid ${C.border}`, borderRadius: 11, background: "#F8FAFF", maxHeight: 220, overflowY: "auto" }}>{options.map((o) => { const checked = value.includes(o); return <button key={o} type="button" onClick={() => onChange(checked ? value.filter((x) => x !== o) : [...value, o])} style={{ border: `1px solid ${checked ? C.accent : C.border}`, borderRadius: 9, padding: "7px 10px", background: checked ? "#EEF2FF" : "#fff", color: checked ? C.accent : C.sub, fontWeight: checked ? 800 : 600, cursor: "pointer", fontSize: 11 }}>{checked ? "✓ " : ""}{o}</button>; })}</div></div>;
}
    ...
    try {
      if (editing) {
        if (isStudent) {
          ...
          await updR("students", editing.id, { name: form.name.trim(), sid: form.sid.trim(), cls: form.cls, sec: form.sec, parentname: form.parentName.trim(), parentphone: form.parentPhone.trim(), enroll: form.enroll, status: form.status });
        ...
        if (!form.cls || !form.sec || !form.parentName?.trim() || !form.parentPhone?.trim()) throw new Error("Fill all required student and parent fields.");
        validatePassword(form.password || DEFAULT_STUDENT_PASSWORD, "Student password");
        const sid = form.sid.trim(); const student = await addR("students", { id: `s-${Date.now()}`, name: form.name.trim(), sid, cls: form.cls, sec: form.sec, parentname: form.parentName.trim(), parentphone: form.parentPhone.trim(), enroll: form.enroll, status: form.status || "active" });
        ...
        if (!form.phone?.trim() || !teacherSubjects.length) throw new Error("Fill Name, Phone, and select at least one Subject.");
        validatePassword(form.password || DEFAULT_TEACHER_PASSWORD, "Teacher password");
