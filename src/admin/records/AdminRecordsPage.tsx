...
async function ensureAdminSession() {
  ...
  const needsRefresh = !session || (expiresAt > 0 && expiresAt <= Math.floor(Date.now() / 1000) + 60);
  ...
  if (session.user.app_metadata?.role !== "admin") {
    throw new Error("Administrator access is required to create student accounts.");
  }
  return session;
}
...
async function provision(role: ProvisionRole, loginId: string, name: string, ref: string, action: "create" | "update" | "delete", authId?: string | null, password?: string) {
  ...
  if (result.error) {
    const message = result.error.message || "Authentication service failed.";
    if (/401|unauthorized|jwt|token|authorization/i.test(message)) {
      await ensureAdminSession();
...
export default function AdminRecordsPage({ kind }: { kind: Kind }) {
  ...
  const openNew = async () => { setEditing(null); setError(""); setSuccess(""); setCredentials(null); setTeacherSubjects([]); setTeacherClasses([]); try { await ensureAdminSession(); if (isStudent) setForm({ name: "", sid: await nextId("students"), tid: "", cls: "", sec: "", enroll: new Date().toISOString().slice(0, 10), status: "active", password: STUDENT_PASSWORD, parentName: "", parentPhone: "", phone: "" }); else setForm({ name: "", sid: "", tid: await nextId("teachers"), cls: "", sec: "", enroll: "", status: "active", password: TEACHER_PASSWORD, parentName: "", parentPhone: "", phone: "" }); setModal(true); } catch (e) { setError(e instanceof Error ? e.message : "Unable to prepare the form."); } };
    ...
    try {
      ...
      if (editing) {
        ...
        try {
          ...
          if (!sa.authId || !pa.authId) throw new Error("Authentication accounts were not returned by Supabase.");
          await updR("students", student.id, { parent: pa.authId });
          await addR("users", { id: `u-${sa.authId}`, name: form.name.trim(), phone: form.sid.trim(), email: sa.email, role: "student", ref: student.id, status: "active", auth_id: sa.authId });
          await addR("users", { id: `u-${pa.authId}`, name: form.parentName.trim(), phone: parentPhone, email: pa.email, role: "parent", ref: student.id, status: "active", auth_id: pa.authId });
          setCredentials({ role: "Student", login: form.sid.trim(), password });
          setSuccess(`Student ${form.name.trim()} created successfully.`);
        } catch (e) {
          for (const item of created.reverse()) {
            if (item.authId) { try { await provision(item.role, item.login, item.name, item.ref, "delete", item.authId); } catch { /* cleanup best effort */ } }
          }
          try { await delR("students", student.id); } catch { /* cleanup best effort */ }
        ...
        await addR("users", { id: `u-${auth.authId}`, name: form.name.trim(), phone, email: auth.email, role: "teacher", ref: teacher.id, status: "active", auth_id: auth.authId });
  ...
  return <section style={{ display: "grid", gap: 16 }}>
    <div style={{ ...card, padding: 20, display: "flex", gap: 12, alignItems: "center", justifyContent: "space-between", flexWrap: "wrap" }}>
      <div><h2 style={{ margin: 0, color: C.text }}>{title}</h2><p style={{ margin: "5px 0 0", color: C.sub, fontSize: 13 }}>{isStudent ? "Manage students and automatically create Student + Parent login accounts." : "Manage teacher profiles and login accounts."}</p></div>
      <button type="button" onClick={() => void openNew()} disabled={loading || saving} style={{ border: 0, borderRadius: 12, padding: "11px 16px", background: C.accent, color: "#fff", fontWeight: 800, cursor: "pointer", opacity: loading || saving ? .6 : 1 }}>{isStudent ? "+ Add Student" : "+ Add Teacher"}</button>
    </div>
    ...
    {credentials && <div style={{ ...card, padding: 16, borderColor: `${C.accent}55`, background: `${C.accent}08` }}><strong>New {credentials.role} login</strong><div style={{ marginTop: 7, fontSize: 13 }}>Login: <b>{credentials.login}</b> · Password: <b>{credentials.password}</b></div></div>}
    ...
    {modal && <div onClick={() => close()} style={{ position: "fixed", inset: 0, background: "rgba(15,27,61,.6)", zIndex: 50, display: "grid", placeItems: "center", padding: 16 }}><div onClick={(e) => e.stopPropagation()} style={{ ...card, width: "min(720px,100%)", maxHeight: "92vh", overflow: "auto", padding: 22 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}><h3 style={{ margin: 0 }}>{editing ? `Edit ${isStudent ? "Student" : "Teacher"}` : `Add ${isStudent ? "Student" : "Teacher"}`}</h3><button type="button" onClick={close} disabled={saving} style={{ border: 0, borderRadius: 9, padding: 8, background: "#F8FAFF", cursor: "pointer" }}>✕</button></div><div style={{ display: "grid", gap: 13 }}><Input label="Full Name" value={form.name} onChange={(v) => setField("name", v)} required />{isStudent ? <><Input label="Student ID" value={form.sid} onChange={(v) => setField("sid", v)} required /><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}><Select label="Class" value={form.cls} onChange={(v) => setField("cls", v)} options={CLASSES} required /><Select label="Section" value={form.sec} onChange={(v) => setField("sec", v)} options={SECTIONS} required /></div><Input label="Enrollment Date" value={form.enroll} onChange={(v) => setField("enroll", v)} type="date" required /><Input label="Parent Name" value={form.parentName} onChange={(v) => setField("parentName", v)} required /><Input label="Parent Phone / Login" value={form.parentPhone} onChange={(v) => setField("parentPhone", v)} required note="Parent login uses this phone number." /><Input label="Student Password" value={form.password} onChange={(v) => setField("password", v)} type="password" required={!editing} note="Minimum 8 characters." /></> : <><Input label="Teacher ID" value={form.tid} onChange={(v) => setField("tid", v)} required /><Input label="Phone / Login" value={form.phone} onChange={(v) => setField("phone", v)} required /><MultiSelect label="Subjects" value={teacherSubjects} onChange={setTeacherSubjects} options={SUBJECTS} /><MultiSelect label="Classes" value={teacherClasses} onChange={setTeacherClasses} options={CLASSES} /><Input label="Teacher Password" value={form.password} onChange={(v) => setField("password", v)} type="password" required={!editing} note="Minimum 8 characters." /></>}</div>{error && <div role="alert" style={{ marginTop: 14, color: C.red }}>{error}</div>}<div style={{ display: "flex", justifyContent: "flex-end", gap: 9, marginTop: 18 }}><button type="button" onClick={close} disabled={saving} style={{ border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 14px", background: "#fff", cursor: "pointer" }}>Cancel</button><button type="button" onClick={() => void save()} disabled={saving} style={{ border: 0, borderRadius: 10, padding: "10px 16px", background: C.accent, color: "#fff", fontWeight: 800, cursor: "pointer", opacity: saving ? .6 : 1 }}>{saving ? "Creating…" : editing ? "Save Changes" : "Create Account"}</button></div></div></div>}
    {confirmDelete && <div onClick={() => !saving && setConfirmDelete(null)} style={{ position: "fixed", inset: 0, background: "rgba(15,27,61,.6)", zIndex: 60, display: "grid", placeItems: "center", padding: 16 }}><div onClick={(e) => e.stopPropagation()} style={{ ...card, width: "min(480px,100%)", padding: 22 }}><h3 style={{ marginTop: 0 }}>Delete {isStudent ? "student" : "teacher"}?</h3><p style={{ color: C.sub }}>This will remove the profile and linked authentication account(s).</p><div style={{ display: "flex", justifyContent: "flex-end", gap: 9 }}><button type="button" onClick={() => setConfirmDelete(null)} disabled={saving} style={{ border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 14px", background: "#fff" }}>Cancel</button><button type="button" onClick={() => void remove(confirmDelete)} disabled={saving} style={{ border: 0, borderRadius: 10, padding: "10px 16px", background: C.red, color: "#fff", fontWeight: 800 }}>{saving ? "Deleting…" : "Delete"}</button></div></div></div>}
  </section>;
}
