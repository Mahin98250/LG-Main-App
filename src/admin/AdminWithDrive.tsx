...
export function AdminWithDrive({ user, onLogout }: { user: AdminUser; onLogout: () => void }) {
  ...
  if (view === "drive") return <div style={{ minHeight: "100vh", background: "#F0F4FF" }}><Header title="Study Materials · Drive" onBack={() => setView("dashboard")} onLogout={onLogout} /><SafeSection title="Study Materials"><MaterialsDrive /></SafeSection></div>;

  if (view === "students" || view === "teachers") {
    const label = view === "students" ? "Students" : "Teachers";
    return <div style={{ minHeight: "100vh", background: "#F0F4FF" }}><Header title={`${label} · Production Management`} onBack={() => setView("dashboard")} onLogout={onLogout} /><SafeSection title={label}><AdminRecordsPage kind={view} /></SafeSection></div>;
  }

  if (view === "batches") return <div style={{ minHeight: "100vh", background: "#F0F4FF" }}><Header title="Batches & Timetable" onBack={() => setView("dashboard")} onLogout={onLogout} /><SafeSection title="Batches & Timetable"><BatchesTimetablePage /></SafeSection></div>;
  if (view === "announcements") return <div style={{ minHeight: "100vh", background: "#F0F4FF" }}><Header title="Announcements" onBack={() => setView("dashboard")} onLogout={onLogout} /><SafeSection title="Announcements"><AnnouncementsPage /></SafeSection></div>;
  if (view === "homework") return <div style={{ minHeight: "100vh", background: "#F0F4FF" }}><Header title="Homework · Production Management" onBack={() => setView("dashboard")} onLogout={onLogout} /><SafeSection title="Homework"><HomeworkPage /></SafeSection></div>;
  if (view === "recovery") return <div style={{ minHeight: "100vh", background: "#F0F4FF" }}><Header title="Password Recovery · Account Security" onBack={() => setView("dashboard")} onLogout={onLogout} /><SafeSection title="User Accounts"><RecoverySettingsPage /></SafeSection></div>;
