...
import { AdminDashboardAnalytics, AdminProfileAnalytics } from "@/admin/AdminAnalytics";
...
export function AdminWithDrive({ user, onLogout }: { user: AdminUser; onLogout: () => void }) {
  ...
  if (view === "dashboard")
    return (
      <SafeSection title="Dashboard Analytics">
        <AdminDashboardAnalytics onBack={() => setView("panel")} />
      </SafeSection>
    );
  if (view === "profiles")
    return (
      <SafeSection title="Profile Analytics">
        <AdminProfileAnalytics onBack={() => setView("panel")} />
      </SafeSection>
    );
  if (view === "drive")
    return (
      <div style={{ minHeight: "100vh", background: "#F0F4FF" }}>
        <Header title="Study Materials · Drive" onBack={() => setView("panel")} />
  ...
  if (view === "students" || view === "teachers") {
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#F0F4FF" }}>
        <Header title={`${label} · Production Management`} onBack={() => setView("panel")} />
  ...
  if (view === "batches")
    return (
      <div style={{ minHeight: "100vh", background: "#F0F4FF" }}>
        <Header title="Batches & Timetable" onBack={() => setView("panel")} />
  ...
  if (view === "announcements")
    return (
      <div style={{ minHeight: "100vh", background: "#F0F4FF" }}>
        <Header title="Announcements" onBack={() => setView("panel")} />
  ...
  if (view === "homework")
    return (
      <div style={{ minHeight: "100vh", background: "#F0F4FF" }}>
        <Header title="Homework · Production Management" onBack={() => setView("panel")} />
  ...
  if (view === "recovery")
    return (
      <div style={{ minHeight: "100vh", background: "#F0F4FF" }}>
        <Header title="Password Recovery · Account Security" onBack={() => setView("panel")} />
