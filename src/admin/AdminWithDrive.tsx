...
const scrollPage: React.CSSProperties = {
  minHeight: "100vh",
  height: "100vh",
  overflowY: "auto",
...
export function AdminWithDrive({
  user,
  onLogout,
}: {
  user: AdminUser;
  ...
  if (view === "profiles") {
    return (
      <SafeSection title="User Profiles">
        <AdminProfilePage onBack={() => setView("dashboard")} />
      </SafeSection>
    );
  }
    ...
    return (
      <div style={scrollPage}>
        <Header
          ...
          onLogout={onLogout}
        />
        <SafeSection title="Tests & Results">
          <TestManagementPage />
        </SafeSection>
      </div>
    );
  ...
  if (view === "homework") {
    return (
      <div style={scrollPage}>
        <Header
          title="Homework · Production Management"
          onBack={() => setView("dashboard")}
          onLogout={onLogout}
        />
