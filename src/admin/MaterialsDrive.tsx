...
function accessLabel(values: string[]) {
  ...
  return selected.length ? selected.map((value) => `Standard ${value}`).join(", ") : "No students selected";
}
...
export function MaterialsDrive() {
  ...
  async function saveAccess() {
    ...
    try {
      const { error: updateError } = await supabase
        .from("material_folders")
        .update({ access_standards: STANDARDS.filter((value) => editingAccessValues.includes(value)) })
  ...
  async function download(file: Material) {
    ...
    try {
      const { data, error: downloadError } = await supabase.storage.from("materials").download(file.storage_path);
    ...
    <div style={wrap}>
      <div style={header}>
        <div>
          ...
          <p style={sub}>Create folders and choose exactly which standards can access them. Batch, section, and subject are not used for access.</p>
        </div>
        <div style={actions}>
          <button type="button" disabled={busy} onClick={() => { setFolderName(""); setFolderAccess(current?.access_standards || []); setNewFolder(true); }} style={btn}>📁 New Folder</button>
          <label style={{ ...btn, cursor: busy ? "wait" : "pointer", opacity: busy || !current ? 0.6 : 1 }}>
            ⬆ Upload
            <input hidden disabled={busy || !current} type="file" accept={ACCEPT} onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file); event.currentTarget.value = ""; }} />
          </label>
      ...
      <div style={crumbRow}>
        ...
        {breadcrumbs.map((folder) => <span key={folder.id}> / <button type="button" onClick={() => setCurrent(folder)} style={crumb}>{folder.name}</button></span>)}
      </div>
      ...
      {current && (
        <div style={hint}>
          ...
          <span style={{ marginLeft: 8, color: "#64748B" }}>Files inside inherit this folder's access.</span>
        </div>
      )}
        ...
        {loading ? <div style={empty}>Loading…</div> : <>
          {currentChildren.map((folder) => (
            <div key={folder.id} style={item}>
              <button type="button" onClick={() => setCurrent(folder)} style={icon}>📁</button>
              ...
              <button type="button" disabled={busy} title="Access" onClick={() => { setEditingAccess(folder); setEditingAccessValues(folder.access_standards || []); }} style={more}>🔐</button>
              <button type="button" disabled={busy} title="Rename" onClick={() => { setRename(folder); setRenameValue(folder.name); }} style={more}>✎</button>
              <button type="button" disabled={busy} title="Delete" onClick={() => void deleteFolder(folder)} style={more}>🗑</button>
            </div>
          ))}
          {currentFiles.map((file) => (
            <div key={file.id} style={item}>
              <div style={{ ...icon, fontSize: 26 }}>{file.mime_type?.includes("pdf") ? "📄" : "📎"}</div>
              <div style={folderMain}>
                <b style={ellipsis}>{file.name || file.title || "Untitled"}</b>
                <span style={meta}>{bytes(file.file_size)}</span>
              </div>
              <button type="button" disabled={busy} title="Download" onClick={() => void download(file)} style={more}>⬇</button>
              <button type="button" disabled={busy} title="Delete" onClick={() => void deleteFile(file)} style={more}>🗑</button>
            </div>
          ))}
          {!currentChildren.length && !currentFiles.length && <div style={{ ...empty, gridColumn: "1/-1" }}>{current ? "This folder is empty. Upload a file here or create a subfolder." : "Create a folder to start your study-material library."}</div>}
        </>}
      </div>

      {newFolder && <div style={overlay}><div style={dialog}>
        <h3 style={{ marginTop: 0 }}>New Folder</h3>
        <input autoFocus value={folderName} onChange={(event) => setFolderName(event.target.value)} placeholder="Folder name" style={input} />
        <div style={{ marginTop: 16, fontWeight: 800 }}>Who can access this folder?</div>
        <div style={standardGrid}>{STANDARDS.map((standard) => <label key={standard} style={check}><input type="checkbox" checked={folderAccess.includes(standard)} onChange={() => toggleStandard(standard, setFolderAccess, folderAccess)} /> Standard {standard}</label>)}</div>
        <p style={note}>All sections and batches inside the selected standard share the same access.</p>
        <div style={dialogActions}><button type="button" onClick={() => setNewFolder(false)} style={cancel}>Cancel</button><button type="button" disabled={busy} onClick={() => void createFolder()} style={btn}>Create</button></div>
      </div></div>}

      {editingAccess && <div style={overlay}><div style={dialog}>
        <h3 style={{ marginTop: 0 }}>Folder Access</h3>
        <p style={sub}>Who can access “{editingAccess.name}”?</p>
        <div style={standardGrid}>{STANDARDS.map((standard) => <label key={standard} style={check}><input type="checkbox" checked={editingAccessValues.includes(standard)} onChange={() => toggleStandard(standard, setEditingAccessValues, editingAccessValues)} /> Standard {standard}</label>)}</div>
        <p style={note}>Changing this applies to the folder and its files. Nested folders inherit access from this folder unless they have their own selected standards.</p>
        <div style={dialogActions}><button type="button" onClick={() => setEditingAccess(null)} style={cancel}>Cancel</button><button type="button" disabled={busy} onClick={() => void saveAccess()} style={btn}>Save Access</button></div>
      </div></div>}

      {rename && <div style={overlay}><div style={dialog}>
        <h3 style={{ marginTop: 0 }}>Rename Folder</h3>
        <input autoFocus value={renameValue} onChange={(event) => setRenameValue(event.target.value)} style={input} />
        <div style={dialogActions}><button type="button" onClick={() => setRename(null)} style={cancel}>Cancel</button><button type="button" disabled={busy} onClick={() => void renameFolder()} style={btn}>Save</button></div>
      </div></div>}
...
const wrap: React.CSSProperties = { minHeight: "100%", padding: 28, fontFamily: "Poppins,system-ui,sans-serif", color: "#0F1B3D" };
const header: React.CSSProperties = { display: "flex", justifyContent: "space-between", gap: 16, flexWrap: "wrap", alignItems: "center", marginBottom: 18 };
const actions: React.CSSProperties = { display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" };
const btn: React.CSSProperties = { border: 0, borderRadius: 11, padding: "10px 14px", background: "#4361EE", color: "#fff", fontWeight: 750, cursor: "pointer" };
const sub: React.CSSProperties = { margin: "5px 0 0", color: "#64748B", fontSize: 13, lineHeight: 1.5 };
const crumbRow: React.CSSProperties = { display: "flex", gap: 7, alignItems: "center", flexWrap: "wrap", marginBottom: 14, fontSize: 13 };
...
const errorBox: React.CSSProperties = { background: "#FEF2F2", color: "#B91C1C", border: "1px solid #FECACA", padding: 12, borderRadius: 12, marginBottom: 14 };
...
const folderMain: React.CSSProperties = { minWidth: 0, flex: 1, border: 0, background: "transparent", textAlign: "left", cursor: "pointer", padding: 0 };
const ellipsis: React.CSSProperties = { display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" };
const meta: React.CSSProperties = { display: "block", color: "#64748B", fontSize: 11, marginTop: 3 };
...
const dialog: React.CSSProperties = { width: "min(460px,100%)", background: "#fff", borderRadius: 20, padding: 24, boxShadow: "0 24px 70px rgba(15,27,61,.25)" };
...
const standardGrid: React.CSSProperties = { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 9, marginTop: 10 };
const check: React.CSSProperties = { display: "flex", alignItems: "center", gap: 8, padding: 10, border: "1px solid #E2E8F0", borderRadius: 10, cursor: "pointer" };
const note: React.CSSProperties = { color: "#64748B", fontSize: 12, lineHeight: 1.5 };
const cancel: React.CSSProperties = { border: "1px solid #CBD5E1", borderRadius: 11, padding: "10px 14px", background: "#fff", cursor: "pointer" };
