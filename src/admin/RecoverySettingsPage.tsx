...
const syntheticEmail = (value: unknown) =>
  String(value || "")
    .trim()
    .toLowerCase()
    .endsWith("@learnersguide.in");
...
export default function RecoverySettingsPage() {
    ...
    try {
      ...
      setDrafts(
        Object.fromEntries(
          accountRows.map((row) => [
            String(row.id),
            syntheticEmail(row.email) ? "" : String(row.email || ""),
          ]),
        ),
      );
    } catch (e) {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      if (!q) return true;
      return [row.name, row.phone, row.email, row.role, row.ref].some((value) =>
        String(value || "")
          .toLowerCase()
          .includes(q),
      );
  ...
  const save = async (row: Row) => {
    ...
    const email = String(drafts[id] || "")
      .trim()
      .toLowerCase();
    setError("");
    setSuccess("");
    if (!isEmail(email)) {
      setError("Enter a valid recovery email address for this account.");
      return;
    }
    setSavingId(id);
    try {
      ...
      setRows((current) =>
        current.map((item) =>
          String(item.id) === id
            ? { ...item, email: result.email || email, auth_id: result.authId }
            : item,
        ),
      );
      setDrafts((current) => ({ ...current, [id]: result.email || email }));
      setSuccess(
        `Recovery email verified and saved for ${row.name || "this account"}. Reset links will be sent only to this address.`,
      );
    } catch (e) {
      setError(
        e instanceof Error
          ? e.message
          : "Unable to save recovery email. The account was not changed.",
      );
    } finally {
      setSavingId(null);
    }
  ...
  return (
    <div style={{ padding: 28, overflowY: "auto", background: "#F7F9FF", minHeight: "100%" }}>
      <div style={{ marginBottom: 18 }}>
        <h2 style={{ margin: 0, color: C.text, fontSize: 22 }}>User Accounts & Recovery</h2>
        <p style={{ margin: "5px 0 0", color: C.sub, fontSize: 13 }}>
          Manage the verified recovery email attached to each real Student, Parent, and Teacher
          Supabase Auth account. Passwords are never displayed or stored here.
        </p>
      </div>
      {success && (
        <div
      ...
      <div style={{ ...card, padding: 16, marginBottom: 18, background: "#F8FAFF" }}>
        ...
        <div style={{ marginTop: 6, color: C.sub, fontSize: 12, lineHeight: 1.6 }}>
          A student starts recovery with their SID. A parent or teacher starts with their registered
          phone number. If the identifier matches an active account with a verified recovery email
          here, Supabase sends the reset link to that email. The email address is never revealed on
          the public recovery screen.
        </div>
      </div>
      <div style={{ display: "flex", gap: 10, marginBottom: 12, flexWrap: "wrap" }}>
        {(["all", "student", "parent", "teacher"] as const).map((role) => (
          <button
            ...
            style={{
              border: `1px solid ${roleFilter === role ? C.accent : C.border}`,
              background: roleFilter === role ? `${C.accent}12` : "#fff",
              color: roleFilter === role ? C.accent : C.sub,
              borderRadius: 10,
              padding: "8px 12px",
              fontWeight: 800,
              cursor: "pointer",
      ...
      >
        <input
          ...
          placeholder="🔍 Search name, SID, phone, email…"
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", minWidth: 980, borderCollapse: "collapse" }}>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={6} style={{ padding: 40, textAlign: "center", color: C.sub }}>
                    Loading real accounts from Supabase…
                  </td>
                </tr>
              ) : filtered.length ? (
                filtered.map((row) => {
                  ...
                  const hasRecovery = Boolean(
                    row.email && !syntheticEmail(row.email) && isEmail(String(row.email)),
                  );
                  const role = String(row.role);
                  return (
                    <tr key={id}>
                      <td style={{ padding: "12px 14px", fontSize: 13, fontWeight: 750 }}>
                      ...
                      <td style={{ padding: "12px 14px", fontSize: 12, fontFamily: "monospace" }}>
                        ...
                        {String(
                          role === "student" ? row.ref || row.phone || "—" : row.phone || "—",
                        )}
                      </td>
                      <td
                        style={{
                          ...
                          fontSize: 12,
                          fontWeight: 800,
                          color: hasRecovery ? "#15803D" : C.red,
                        }}
                      >
                        {hasRecovery ? "✓ Verified" : "Not configured"}
                      </td>
                      <td style={{ padding: "12px 14px", minWidth: 290 }}>
                        <input
                          type="email"
                          value={drafts[id] ?? ""}
                          onChange={(e) =>
                            setDrafts((current) => ({ ...current, [id]: e.target.value }))
                          }
                          onKeyDown={(e) => handleEmailKeyDown(e, row)}
                          placeholder={hasRecovery ? "Recovery email" : "Add recovery email"}
                          aria-label={`Recovery email for ${String(row.name || "User")}`}
                          style={{
                            width: "100%",
