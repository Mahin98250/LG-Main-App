...
export default function AdminLogin({ onAuthenticated }: AdminLoginProps) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // The role must come from Supabase app_metadata, never from the browser.
  ...
  if (checkingSession) {
    return (
      <main className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-6">
        <div className="text-sm text-slate-300">Checking administrator session...</div>
      </main>
    );
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-5" noValidate>
          <label className="block">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-white outline-none transition placeholder:text-slate-500 focus:border-blue-400 focus:ring-2 focus:ring-blue-400/20"
              disabled={loading}
            />
          </label>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full rounded-2xl bg-blue-500 px-4 py-3.5 font-semibold text-white transition hover:bg-blue-400 disabled:cursor-not-allowed disabled:opacity-60"
          >
