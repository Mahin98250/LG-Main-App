...
const authEmail = (role: string, id: string) =>
  String(id).includes("@")
    ? String(id).trim().toLowerCase()
    : `${({ teacher: "t", student: "s", parent: "p", admin: "u" } as Record<string, string>)[role] || "u"}.${normalize(id)}@learnersguide.in`;
...
async function findUserByRefRole(a: ReturnType<typeof createClient>, ref: string | null, role: string) {
  ...
  return (
    data.users.find(
      (u) => u.app_metadata?.role === role && String(u.app_metadata?.ref || "") === String(ref),
    ) || null
  );
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (callerAdminError || !callerAdmin.user) {
      return json({ error: "Unable to verify administrator" }, 500);
    }
    if (callerAdmin.user.app_metadata?.role !== "admin") {
    ...
    if (action === "create") {
      ...
      if (existingByEmail) {
        ...
        if (existingRole && existingRole !== role) {
          return json({ error: "This login ID is already used by another account type." }, 409);
        }
        if (existingRef && ref && String(existingRef) !== String(ref)) {
          return json({ error: "This login ID is already linked to another institute profile." }, 409);
        }

        const { data, error } = await admin.auth.admin.updateUserById(existingByEmail.id, {
          password,
          user_metadata: { ...(existingByEmail.user_metadata || {}), name, phone: loginId },
          app_metadata: { ...(existingByEmail.app_metadata || {}), role, ref },
        });
      ...
      const { data, error } = await admin.auth.admin.createUser({
        ...
        password,
        email_confirm: true,
        user_metadata: { name, phone: loginId },
        app_metadata: { role, ref },
      });
    ...
    if (!user) {
      if (!password) {
        return json(
          { error: "Authentication account not found. A password is required to recreate it.", code: "AUTH_ACCOUNT_MISSING" },
          404,
        );
      }
    ...
    if (existingRole && existingRole !== role) {
      return json({ error: "The authentication account belongs to a different role." }, 409);
    }

    const patch: Record<string, unknown> = {
      user_metadata: { ...(user.user_metadata || {}), name, phone: loginId },
      app_metadata: { ...(user.app_metadata || {}), role, ref },
    };
